...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  UPDATE public.inbox_outbox_jobs SET status = 'processing', attempt_count = attempt_count + 1, updated_at = now()
  WHERE workspace_id = p_workspace_id AND id = p_id AND status = 'pending' RETURNING *;
$$;
