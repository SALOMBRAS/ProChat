...
func (s *server) doStartCall(sess *Session, w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil || (strings.TrimSpace(body.Phone) == "" && strings.TrimSpace(body.LID) == "") {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "phone or lid required"})
		return
	}
	owner := clientID(r)
	...
	var peer types.JID
	if lid := normalizePhone(body.LID); lid != "" {
		// Contato endereçado por LID (WhatsApp moderno esconde o número real).
		// Caminho preferido: o store da sessão já conhece o PN deste LID e a
		// chamada segue pelo número canônico, que é o fluxo validado. Sem mapa,
		// disca o JID @lid direto — o protocolo atual endereça por LID.
		peer = types.NewJID(lid, types.HiddenUserServer)
		if pn, err := sess.client.Store.GetAltJID(r.Context(), peer); err == nil && !pn.IsEmpty() {
			s.log.Info("LID resolved to phone", "lid", peer.String(), "phone", pn.String())
			peer = pn.ToNonAD()
		}
	} else {
	...
	// Resolve o JID canônico antes de chamar: números BR móveis podem estar
	// registrados sem o 9º dígito (ex.: discado 55859XXXXXXXX, JID real
	// 5585XXXXXXXX). Chamar o JID errado faz o WhatsApp descartar o offer em
	// silêncio (0 dispositivos no USync, sem ack — "ringing" eterno). Só faz
	// sentido para número discado: LID já é um identificador de protocolo.
	if peer.Server == types.DefaultUserServer {
		if resp, err := sess.client.IsOnWhatsApp(r.Context(), []string{peer.User}); err == nil && len(resp) > 0 {
			if !resp[0].IsIn {
				writeJSON(w, http.StatusBadRequest, map[string]string{"error": "number is not on WhatsApp"})
				return
			}
			if canon := resp[0].JID; !canon.IsEmpty() && canon.User != peer.User {
				s.log.Info("canonical JID resolved", "dialed", peer.String(), "canonical", canon.String())
				peer = canon.ToNonAD()
			}
		}
	}
