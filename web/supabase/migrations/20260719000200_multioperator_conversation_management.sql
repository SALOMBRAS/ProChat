...
CREATE TABLE IF NOT EXISTS public.conversation_events (
  id uuid NOT NULL DEFAULT gen_random_uuid(), conversation_id uuid NOT NULL, workspace_id text NOT NULL, user_id uuid NOT NULL,
  action text NOT NULL CHECK (action IN ('assigned', 'unassigned', 'status_changed', 'priority_changed', 'archived', 'reopened')),
  previous_value text NULL, new_value text NULL, created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (workspace_id, id), FOREIGN KEY (workspace_id, conversation_id) REFERENCES public.conversations(workspace_id, id) ON DELETE CASCADE
);
