...
export type HistorySyncJob = { id: string; jobId: string; wahaSession: string; status: 'pending' | 'running' | 'completed' | 'failed' | 'cancelled'; chatsProcessed: number; messagesProcessed: number; chatsTotal: number | null; currentChat: string | null; hasMore: boolean; progressLabel: string; lastErrorSafe: string | null; updatedAt: string };
...
export type KanbanCard = { conversationId:string; maskedId:string; identity:InboxConversation['identity']|null; lastMessage:string; lastMessageAt:string; unreadCount:number; conversationType:'direct'|'group'; assignedUserId:string|null; assignedTeamId:string|null; routingQueueId:string|null; priority:ConversationPriority; tags:string[]; slaStatus:string|null; sla:KanbanSla|null; stageId:string; position:number; updatedAt:string };
