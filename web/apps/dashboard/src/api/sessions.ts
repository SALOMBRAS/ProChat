...
export class SessionsApi {
  ...
  list=()=>this.http.get<Session[]>('/api/v1/sessions'); create=(name:string, clientRequestId = crypto.randomUUID())=>this.http.post<Session>('/api/v1/sessions',{name, clientRequestId}); status=(id:string, signal?:AbortSignal)=>this.http.get<Session>(`/api/v1/sessions/${encodeURIComponent(id)}/status`, signal); qr=(id:string, signal?:AbortSignal)=>this.http.get<SessionQr>(`/api/v1/sessions/${encodeURIComponent(id)}/qr`, signal); connect=(id:string,forceQrRefresh=false)=>this.http.post<void>(`/api/v1/sessions/${encodeURIComponent(id)}/connect`,{forceQrRefresh}); stop=(id:string)=>this.http.post<void>(`/api/v1/sessions/${encodeURIComponent(id)}/stop`); logout=(id:string)=>this.http.post<void>(`/api/v1/sessions/${encodeURIComponent(id)}/logout`); remove=(id:string)=>this.http.delete(`/api/v1/sessions/${encodeURIComponent(id)}`);
}
