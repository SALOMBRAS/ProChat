...
export class WahaProvider implements WhatsAppWorkerPort {
  ...
  private async persist(): Promise<void> { await this.registry.save([...[...this.sessions.values()].map(({ id, workspaceId, name, wahaName, aliases }) => ({ sessionId: id, workspaceId, name, wahaName, ...(aliases.length ? { aliases } : {}) })), ...[...this.retired.values()].flat()]); }
