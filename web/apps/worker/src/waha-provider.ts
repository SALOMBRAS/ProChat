...
import { assertSafeIdentifier } from './identifiers.js'; import { WorkerOperationError, type WorkerCommand } from './ports.js'; import { CommandBackedWhatsAppProvider } from './provider-operations.js'; import { detached } from './request-deadline.js'; import { WahaClientError, type WahaClientPort, type WahaSession } from './waha-client.js'; import type { WahaSessionRegistryEntry } from './waha-session-registry.js';
...
export class WahaProvider extends CommandBackedWhatsAppProvider {
  readonly provider = 'waha' as const;
  // `sendContent` names what WAHA already did through the sendContent command
  // (location and vCard); it is a new label, not a new behaviour.
  readonly capabilities = ['health', 'sessions', 'status', 'sendMessage', 'sendMedia', 'sendContent', 'contacts', 'groups', 'reactions', 'history'] as const;
  private readonly sessions = new Map<string, StoredSession>(); private readonly pendingCreates = new Map<string, Promise<void>>(); private readonly orphans = new Map<string, WahaSession[]>(); private readonly retired = new Map<string, WahaSessionRegistryEntry[]>(); private restored = false;
  constructor(private readonly client: WahaClientPort, private readonly qrTtlMs: number, private readonly registry: Registry = { load: async () => [], save: async () => undefined }) { super(); }
  async execute(context: RequestContext, command: WorkerCommand) { await this.restore(); if (command.type === 'listSessions') { await this.reconcile(context.workspaceId); return [...[...this.sessions.values()].filter(session => session.workspaceId === context.workspaceId).map(session => this.summary(session)), ...(this.orphans.get(context.workspaceId) ?? []).map(remote => this.orphan(context.workspaceId, remote))]; } if (command.type === 'createSession') return this.create(context, command.sessionId, command.input); if (command.type === 'sendMessage') return this.sendTextCommand(context, command.wahaSession, command.chatId, command.text, command.mentions, command.linkPreview); if (command.type === 'sendAttachment') return this.sendAttachment(context, command.wahaSession, command.chatId, command.attachment); if (command.type === 'sendContent') return this.sendContent(context, command.wahaSession, command.chatId, command.content); if (command.type === 'sendReaction') return this.sendReaction(context, command.wahaSession, command.chatId, command.messageId, command.reaction); if (command.type === 'syncIdentity') return this.syncIdentity(context, command); if (command.type === 'historyPage') return this.historyPage(context, command); if (command.type === 'contactsPage') return this.contactsPage(context, command); if (command.type === 'lidsPage') return this.lidsPage(context, command); if (command.type === 'mergeSessionAliases') return this.mergeAliases(context, command.sessionId, command.aliases); const stored = this.require(context, command.sessionId); if (command.type === 'getSession') return this.refresh(context, stored); if (command.type === 'connectSession') { await this.pendingCreates.get(this.key(context.workspaceId, stored.id)); await this.call(context, () => this.client.startSession(stored.wahaName)); await this.refresh(context, stored); return; } if (command.type === 'disconnectSession') { await this.call(context, () => this.client.stopSession(stored.wahaName)); this.setStatus(stored, 'stopped'); return; } if (command.type === 'logoutSession') { await this.call(context, () => this.client.logoutSession(stored.wahaName)); this.setStatus(stored, 'disconnected'); return; } if (command.type === 'getQr') return this.qr(context, stored); await this.call(context, () => this.client.removeSession(stored.wahaName)); this.sessions.delete(this.key(context.workspaceId, stored.id)); const retired = this.retired.get(context.workspaceId) ?? []; retired.push({ workspaceId: stored.workspaceId, sessionId: stored.id, name: stored.name, wahaName: stored.wahaName, ...(stored.aliases.length ? { aliases: stored.aliases } : {}), deletedAt: new Date().toISOString() }); this.retired.set(context.workspaceId, retired); await this.persist(); }
  ...
  private async create(context: RequestContext, sessionId: string, input: { name?: string }): Promise<WhatsAppSession> {
    ...
    // Workspace sem nenhuma outra sessão linkada é o caso de conta única: o
    // operador removeu o dispositivo anterior e está pareando de novo. Herdar
    // os nomes aposentados como aliases mantém alcançáveis as conversas
    // gravadas com eles. Com outra sessão ativa no workspace não há como saber
    // qual número está sendo repareado, então nada é herdado — o guarda de
    // sessão inativa existe justamente para não mandar mensagem pela conta
    // errada.
    const retiredEntries = this.retired.get(valid.workspaceId) ?? [];
    if (![...this.sessions.values()].some(session => session.workspaceId === valid.workspaceId) && retiredEntries.length) {
      stored.aliases = [...new Set(retiredEntries.flatMap(entry => [entry.wahaName, ...(entry.aliases ?? [])]))].filter(name => name !== stored.wahaName);
      this.retired.delete(valid.workspaceId);
    }
  ...
  private async restore(): Promise<void> { if (this.restored) return; for (const entry of await this.registry.load()) { if (entry.deletedAt) { const list = this.retired.get(entry.workspaceId) ?? []; list.push(entry); this.retired.set(entry.workspaceId, list); continue; } const now = new Date().toISOString(); const stored: StoredSession = { ...whatsAppSessionSchema.parse({ id: entry.sessionId, workspaceId: entry.workspaceId, name: entry.name, status: 'disconnected', createdAt: now, updatedAt: now }), wahaName: entry.wahaName, aliases: entry.aliases ?? [], ...(entry.phone ? { phone: entry.phone } : {}) }; this.sessions.set(this.key(entry.workspaceId, entry.sessionId), stored); } this.restored = true; }
  private async persist(): Promise<void> { await this.registry.save([...[...this.sessions.values()].map(({ id, workspaceId, name, wahaName, aliases, phone }) => ({ sessionId: id, workspaceId, name, wahaName, ...(aliases.length ? { aliases } : {}), ...(phone ? { phone } : {}) })), ...[...this.retired.values()].flat()]); }
  ...
  private async reconcile(workspaceId: string): Promise<void> {
    ...
    for (const session of linked) {
      if (this.sessions.get(this.key(workspaceId, session.id)) !== session) continue; // fundida por assignPhone neste mesmo reconcile
      const current = remoteByName.get(session.wahaName);
      if (current) { this.setStatus(session, statusMap[current.status.toUpperCase()] ?? 'error'); if (current.me) await this.assignPhone(workspaceId, session, current.me.id); continue; }
      ...
      const replacement = candidates[0]; session.aliases = [...new Set([...session.aliases, session.wahaName])]; session.wahaName = replacement.name; claimed.add(replacement.name); this.setStatus(session, statusMap[replacement.status.toUpperCase()] ?? 'error'); if (replacement.me) await this.assignPhone(workspaceId, session, replacement.me.id); changed = true;
    }
  ...
  /** Número próprio aprendido da WAHA (`me.id`): grava na entry e funde outras
   *  entries do workspace com o MESMO número — cada uma vira alias desta, que é
   *  a viva. É o que impede a duplicação de instância quando o mesmo WhatsApp
   *  reconecta num registry novo (outra máquina, reinstalação). */
  private async assignPhone(workspaceId: string, stored: StoredSession, meId: string): Promise<void> {
    const phone = meId.split('@', 1)[0]!.split(':', 1)[0]!.replace(/\D/g, '');
    if (!phone || stored.phone === phone) return;
    stored.phone = phone;
    for (const other of [...this.sessions.values()]) {
      if (other === stored || other.workspaceId !== workspaceId || other.phone !== phone) continue;
      stored.aliases = [...new Set([...stored.aliases, other.wahaName, ...other.aliases])].filter(name => name !== stored.wahaName);
      this.sessions.delete(this.key(other.workspaceId, other.id));
    }
    await this.persist();
  }
  /** Aliases descobertos pela API no histórico (nomes WAHA de outras máquinas
   *  com o mesmo número): entram na entry viva sem tocar em dado persistido. */
  private async mergeAliases(context: RequestContext, sessionId: string, aliases: string[]): Promise<void> {
    const stored = this.require(context, sessionId);
    const additions = aliases.filter(name => name !== stored.wahaName && !stored.aliases.includes(name));
    if (!additions.length) return;
    stored.aliases = [...stored.aliases, ...additions];
    await this.persist();
  }
  private matchesWaha(session: StoredSession, name: string): boolean { return session.wahaName === name || session.aliases.includes(name); }
  private async refresh(context: RequestContext, stored: StoredSession): Promise<WhatsAppSession> { if (this.pendingCreates.has(this.key(context.workspaceId, stored.id))) return this.summary(stored); try { const external = await this.client.getSession(stored.wahaName); this.setStatus(stored, statusMap[external.status.toUpperCase()] ?? 'error'); if (external.me) await this.assignPhone(context.workspaceId, stored, external.me.id); } catch (error) { if (!(error instanceof WahaClientError) || error.status !== 404) throw await this.call(context, async () => { throw error; }); await this.reconcile(context.workspaceId); if (stored.status === 'error') throw new WorkerOperationError('NOT_FOUND', 'Linked WAHA session was not found and could not be reconciled safely', context.correlationId); } return this.summary(stored); }
