...
export class WahaProvider implements WhatsAppWorkerPort {
  ...
  private async persist(): Promise<void> { await this.registry.save([...this.sessions.values()].map(({ id, workspaceId, name, wahaName, aliases, phone }) => ({ sessionId: id, workspaceId, name, wahaName, ...(aliases.length ? { aliases } : {}), ...(phone ? { phone } : {}) }))); }
