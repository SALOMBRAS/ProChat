...
export class DemoWhatsAppWorkerAdapter implements WhatsAppWorkerPort {
  ...
  async execute(context: RequestContext, command: WorkerCommand) {
    ...
    if (command.type === 'sendMessage' || command.type === 'sendAttachment' || command.type === 'sendReaction') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Manual messaging is unavailable in demo mode', context.correlationId);
