...
export class DemoWhatsAppWorkerAdapter implements WhatsAppWorkerPort {
  ...
  async execute(context: RequestContext, command: WorkerCommand) {
    ...
    if (command.type === 'sendMessage' || command.type === 'sendAttachment') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Manual messaging is unavailable in demo mode', context.correlationId);
    ...
    if (command.type === 'contactsPage') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Contact synchronization is unavailable in demo mode', context.correlationId);
    if (command.type === 'lidsPage') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Contact synchronization is unavailable in demo mode', context.correlationId);
    if (command.type === 'sendContent') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Location, contact card and poll are unavailable in demo mode', context.correlationId, { kind: command.content.kind });
    if (command.type === 'sendReaction') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Message reactions are unavailable in demo mode', context.correlationId);
