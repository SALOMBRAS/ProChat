...
export type WahaSessionRegistryEntry = { workspaceId: string; sessionId: string; name: string; wahaName: string; aliases?: string[]; deletedAt?: string; phone?: string };
...
function isEntry(value: unknown): value is WahaSessionRegistryEntry { if (!value || typeof value !== 'object') return false; const record = value as Record<string, unknown>; const aliases = record.aliases; const deletedAt = record.deletedAt; return ['workspaceId', 'sessionId', 'name', 'wahaName'].every(key => typeof record[key] === 'string' && String(record[key]).length > 0) && (aliases === undefined || (Array.isArray(aliases) && aliases.every((item: unknown) => typeof item === 'string' && item.length > 0))) && (deletedAt === undefined || (typeof deletedAt === 'string' && deletedAt.length > 0)) && (record.phone === undefined || (typeof record.phone === 'string' && record.phone.length > 0)); }
