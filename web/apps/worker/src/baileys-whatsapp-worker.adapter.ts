...
export class BaileysWhatsAppWorkerAdapter implements WhatsAppWorkerPort {
  ...
  async execute(context: RequestContext, command: WorkerCommand) {
    ...
    if (command.type === 'sendMessage' || command.type === 'sendAttachment' || command.type === 'sendReaction') throw new WorkerOperationError('NOT_IMPLEMENTED', 'Manual messaging is available only for the WAHA provider', context.correlationId);
