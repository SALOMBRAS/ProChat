...
describe('WhatsAppHistorySyncService', () => {
  it('shares one active run between concurrent starts', async () => {
    const store = new MemoryStore(); const send = vi.fn().mockResolvedValue(response([])); const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,   });
  ...
  it('does not retry permanent worker failures and preserves the checkpoint', async () => {
    const store = new MemoryStore(); const send = vi.fn().mockResolvedValue(failed('NOT_FOUND')); const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,   });
  ...
  it('continues automatically after a batch checkpoint until the full history is complete', async () => {
    ...
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  maxMessagesPerRun: 2, continuationDelayMs: 1, sleep });
  ...
  it('preserves the checkpoint at the emergency execution guard', async () => {
    ...
    const sleep = vi.fn().mockResolvedValue(undefined);
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  emergencyMaxMessages: 2, sleep });
    await service.start('workspace-a', 'session-a');
  ...
  it('closes a chat that keeps timing out and still finishes the remaining history', async () => {
    ...
    const sleep = vi.fn().mockResolvedValue(undefined);
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep, retryBaseMs: 1 });
    await service.start('workspace-a', 'session-a');
    await waitFor(() => store.job?.status === 'completed');
  ...
  it('stops retrying a timeout that is deep inside a chat, because the cost is the offset itself', async () => {
    ...
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep, retryBaseMs: 1, messagePageSize: 1 });
    await service.start('workspace-a', 'session-a');
    await waitFor(() => store.job?.status === 'completed');
  ...
  it('still retries a timeout on the first page of a chat, where the cost does not depend on the offset', async () => {
    ...
    const sleep = vi.fn().mockResolvedValue(undefined);
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep, retryBaseMs: 10 });
    await service.start('workspace-a', 'session-a');
    await waitFor(() => store.job?.status === 'completed');
  ...
  it('resumes a failed job from its checkpoint instead of restarting the history', async () => {
    const store = new MemoryStore();
    store.job = { id: 'job-a', workspaceId: 'workspace-a', wahaSession: 'session-a', status: 'failed', currentChatId: '120363@g.us', chatCursor: '4', messageCursor: '300', chatsProcessed: 4, chatsTotal: null, messagesProcessed: 300, startedAt: '2026-07-22T14:00:38.458Z', completedAt: null, lastErrorSafe: 'TIMEOUT', updatedAt: '2026-07-24T13:10:34.089Z' };
    const send = vi.fn().mockImplementation((request: any) => {
      const { chatId, offset } = request.command.payload;
      if (!chatId) return response([]);
      return offset === 300 ? response([{ id: 'm-1', chatId: '120363@g.us', timestamp: 1 }]) : response([]);
    });
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep: vi.fn().mockResolvedValue(undefined) });
  ...
  it('adopts a job abandoned as running by an interrupted process, and refuses to duplicate a live one', async () => {
    const store = new MemoryStore();
    const running = { id: 'job-a', workspaceId: 'workspace-a', wahaSession: 'session-a', status: 'running' as const, currentChatId: null, chatCursor: '7', messageCursor: null, chatsProcessed: 7, chatsTotal: null, messagesProcessed: 70, startedAt: '2026-07-22T14:00:38.458Z', completedAt: null, lastErrorSafe: null, updatedAt: new Date(Date.now() - 600_000).toISOString() };
    const send = vi.fn().mockResolvedValue(response([]));
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep: vi.fn().mockResolvedValue(undefined) });
    store.job = { ...running };
    await service.start('workspace-a', 'session-a');
    await waitFor(() => store.job?.status === 'completed');
  ...
  it('records which timeout actually happened instead of a bare code, and never names a deadline of its own', async () => {
    ...
    for (const cause of causes) {
      ...
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep: vi.fn().mockResolvedValue(undefined), retryBaseMs: 1 });
      await service.start('workspace-a', 'session-a');
  ...
  it('stops between two messages of a page when the run is cancelled', async () => {
    ...
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep: vi.fn().mockResolvedValue(undefined) });
    await service.start('workspace-a', 'session-a');
  ...
  it('asks WAHA for one chat listing per page instead of one per conversation', async () => {
    ...
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep: vi.fn().mockResolvedValue(undefined), maxChatsPerRun: 100 });
    await service.start('workspace-a', 'session-a');
    await waitFor(() => store.job?.status === 'completed');
  ...
  it('does not walk a conversation twice when an arriving message reorders the listing', async () => {
    ...
    // Três conversas andadas, mas CINCO posições consumidas: o reordenamento pôs
    // 1 e 2 de novo no caminho do cursor, e cada reencontro é uma posição.
    // `chatsProcessed` conta posições, não conversas distintas — é o que faz a
    // fração fechar, e é por isso que a porcentagem é presa em 100%: a listagem
    // anda por baixo do total contado no começo.
    expect(store.job).toMatchObject({ status: 'completed', chatsProcessed: 5, chatCursor: '5', messagesProcessed: 3 });
  });

  describe('counting the session chats for the progress denominator', () => {
    // WAHA 2026.7.1 has no chats count endpoint: /chats is a bare array, and
    // /groups/count and /lids/count have no chats sibling. The service finds the
    // total by binary search over `offset` with a one-item page, so a probe
    // transfers one chat instead of the whole listing.
    const listing = (total: number) => (request: any) => {
      const { chatId, offset, limit } = request.command.payload;
      if (chatId) return response([{ id: `m-${chatId}`, chatId, timestamp: 1 }]);
      const remaining = Math.max(0, total - offset);
      const items = Array.from({ length: Math.min(limit, remaining) }, (_unused, index) => ({ id: `${offset + index + 1}@c.us` }));
      // `hasMore` is measured before the filter, which is what makes a probe a
      // question about the position existing.
      return response(items, items.length === limit);
    };

    it('finds the exact total, and asks for one chat per probe', async () => {
      const store = new MemoryStore(); const send = vi.fn().mockImplementation(listing(551));
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { sleep: vi.fn().mockResolvedValue(undefined), maxChatsPerRun: 1 });
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.chatsTotal !== null);
      expect(store.job?.chatsTotal).toBe(551);
      const probes = send.mock.calls.filter((call: any[]) => !call[0].command.payload.chatId && call[0].command.payload.limit === 1);
      // Ramp to 1024 then bisect: about 2 log2(n), not 551 / page size.
      expect(probes.length).toBeLessThan(25);
      expect(probes.every((call: any[]) => call[0].command.payload.limit === 1)).toBe(true);
    });

    it('counts a position held by a filtered-out chat, because the probe asks hasMore', async () => {
      // `status@broadcast` ocupa uma posição real e é descartado pelo filtro de
      // `listChats`. A sonda pergunta se a POSIÇÃO existe (`hasMore`, medido antes
      // do filtro). Perguntando `items.length`, a contagem pararia nessa posição e
      // devolveria um total curto — com a ordenação por recência o broadcast pode
      // cair no offset 0 e o total sairia zero.
      const store = new MemoryStore();
      const send = vi.fn().mockImplementation((request: any) => {
        const { chatId, offset, limit } = request.command.payload;
        if (chatId) return response([]);
        if (limit === 1) return offset === 1 ? response([], true) : offset < 3 ? response([{ id: `${offset + 1}@c.us` }], true) : response([], false);
        return offset === 0 ? response([{ id: '1@c.us' }, { id: '3@c.us' }]) : response([]);
      });
      ...
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(store.job?.chatsTotal).toBe(3);
    });

    it('counts a whole page that yielded no usable chat as walked', async () => {
      // A página inteira caiu no filtro: o cursor pula o tamanho da página, e a
      // fração só fecha se o numerador pular o mesmo tanto.
      const store = new MemoryStore();
      const send = vi.fn().mockImplementation((request: any) => {
        const { chatId, offset, limit } = request.command.payload;
        if (chatId) return response([]);
        if (limit === 1) return offset < 6 ? response([{ id: 'x@c.us' }], true) : response([], false);
        return offset === 0 ? response([], true) : offset === 5 ? response([{ id: '6@c.us' }]) : response([]);
      });
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { chatPageSize: 5, sleep: vi.fn().mockResolvedValue(undefined) });
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(store.job?.chatsTotal).toBe(6);
      // 5 posições da página descartada + a conversa que veio depois.
      expect(store.job).toMatchObject({ chatsProcessed: 6, chatCursor: '6' });
    });

    it('counts an empty session as zero, not as unknown', async () => {
      const store = new MemoryStore(); const send = vi.fn().mockImplementation(listing(0));
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub);
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(store.job?.chatsTotal).toBe(0);
    });

    it('runs without a denominator when the count fails, instead of failing the job', async () => {
      // Falha aberta: a corrida vale mais que a barra de progresso.
      const store = new MemoryStore();
      let counted = false;
      const send = vi.fn().mockImplementation((request: any) => {
        if (!request.command.payload.chatId && request.command.payload.limit === 1 && !counted) { counted = true; return failed('NOT_FOUND'); }
        return listing(1)(request);
      });
      ...
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(store.job?.chatsTotal).toBeNull();
      expect(store.job?.messagesProcessed).toBe(1);
    });

    it('gives up counting past the sanity ceiling rather than probing forever', async () => {
      // Um provedor que responde `hasMore` sempre sondaria sem fim.
      const store = new MemoryStore();
      // A sonda (limit 1) nunca chega ao fim; a caminhada, sim — senão a corrida
      // não terminaria e o teste mediria outra coisa.
      const send = vi.fn().mockImplementation((request: any) => {
        const { chatId, limit, offset } = request.command.payload;
        if (chatId) return response([]);
        if (limit === 1) return response([{ id: 'x@c.us' }], true);
        return offset === 0 ? response([{ id: '1@c.us' }]) : response([]);
      });
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 64, sleep: vi.fn().mockResolvedValue(undefined) });
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(store.job?.chatsTotal).toBeNull();
      // E parou de sondar: a rampa dobra, então o teto de 64 custa ~7 sondas.
      expect(send.mock.calls.filter((call: any[]) => call[0].command.payload.limit === 1).length).toBeLessThan(12);
    });

    it('recounts a fresh run and keeps the total across a resume', async () => {
      const store = new MemoryStore(); const send = vi.fn().mockImplementation(listing(3));
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { sleep: vi.fn().mockResolvedValue(undefined) });
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(store.job?.chatsTotal).toBe(3);

      // Uma corrida nova conta de novo: a conta pode ter crescido.
      store.job = { ...store.job!, status: 'completed' };
      send.mockImplementation(listing(5));
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.chatsTotal === 5);
      expect(store.job?.chatsTotal).toBe(5);
    });

    it('the status projection and the realtime event both carry the total', async () => {
      const store = new MemoryStore(); const publish = vi.fn(); const send = vi.fn().mockImplementation(listing(2));
      const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn().mockResolvedValue({ duplicate: false }) } as unknown as WahaWebhookStore, store, { publish } as unknown as RealtimeHub, { sleep: vi.fn().mockResolvedValue(undefined) });
      await service.start('workspace-a', 'session-a');
      await waitFor(() => store.job?.status === 'completed');
      expect(await service.status('workspace-a', 'session-a')).toMatchObject({ chatsTotal: 2 });
      expect(publish.mock.calls.some((call: any[]) => call[2]?.chatsTotal === 2)).toBe(true);
    });
  });
  ...
  it('fails the job when consecutive chats time out, instead of marking them processed', async () => {
    ...
    const sleep = vi.fn().mockResolvedValue(undefined);
    const service = new WhatsAppHistorySyncService({ send } as unknown as InternalWorkerClient, { ingest: vi.fn() } as unknown as WahaWebhookStore, store, { publish: vi.fn() } as unknown as RealtimeHub, { maxCountedChats: 0,  sleep, retryBaseMs: 1, maxConsecutiveChatTimeouts: 2 });
    await service.start('workspace-a', 'session-a');
    await waitFor(() => store.job?.status === 'failed');
