...
describe('API foundation', () => {
  ...
  it('returns standard not-implemented error after workspace context without user authentication', async () => { const response = await request(createApp()).get('/api/v1/sessions').set('x-workspace-id','workspace-a').expect(501); expect(response.body.error.code).toBe('NOT_IMPLEMENTED'); });
