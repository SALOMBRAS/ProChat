...
describe('WAHA webhook ingress', () => {
  ...
  it('returns chronological message history and marks only the workspace conversation as read', async () => {
    ...
    await request(app).get(`/api/v1/inbox/conversations/${id}/messages?page=1&pageSize=1`).set('x-workspace-id', 'workspace-a').expect(200).expect(response => expect(response.body).toMatchObject({ total: 2, hasMore: true, items: [{ id: 'message-history-2', direction: 'outbound', content: 'Resposta', status: 'sent' }] }));
