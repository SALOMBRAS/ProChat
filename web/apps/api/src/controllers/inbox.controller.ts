...
export class InboxController {
  ...
  listConversations: RequestHandler = async (req, res) => { const input = query.parse(req.query); const page = await this.conversations.listConversations(req.context!.workspaceId, input.page, input.pageSize, input.cursor, input.search); res.json({ ...page, items: withSessionActivity(page.items, await this.activeSessions(req.context!)) }); };
  getConversation: RequestHandler = async (req, res) => { const conversationId = z.string().uuid().parse(req.params.conversationId); const conversation = await this.conversations.getConversation(req.context!.workspaceId, conversationId); if (!conversation) throw new AppError(404, 'NOT_FOUND', 'Conversation not found'); res.json(withSessionActivity([conversation], await this.activeSessions(req.context!))[0]); };
