...
export async function createApp(config: ApiConfig = loadConfig()) {
  ...
  try {
    ...
    webhookStore = database ? new SqliteWahaWebhookStore(database.sqlite, kanbanAutomation, config.ownWhatsappNumbers) : new SupabaseWahaWebhookStore(supabase!, kanbanAutomation, config.ownWhatsappNumbers);
