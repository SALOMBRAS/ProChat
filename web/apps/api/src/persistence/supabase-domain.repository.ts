...
export class SupabaseDomainRepository implements DomainRepository {
  ...
  private async count(table: string, workspaceId: string, visibleOnly = false): Promise<number> { let query = this.client.from(table).select('*', { count: 'exact', head: true }).eq('workspace_id', workspaceId); if (visibleOnly) query = query.eq('visibility_state', 'visible'); const { count, error: queryError } = await query; error(queryError); return count ?? 0; }
