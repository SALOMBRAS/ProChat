...
export function attachWebSocket(server: Server, hub: RealtimeHub): WebSocketServer {
  ...
  wss.on('connection', (socket, request) => {
    ...
    hub.add(socket, workspaceId, userId); socket.once('close', () => hub.remove(socket)); socket.send(JSON.stringify(event)); log('info', 'WebSocket connected', { workspaceId, userId, correlationId: event.correlationId });
  });
  return wss;
}
