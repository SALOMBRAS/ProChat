...
/** `providerSessionId` é opcional durante a transição: o ingresso GOWA já o
 *  resolve e grava, o da WAHA ainda não tem linha em `whatsapp_provider_sessions`
 *  e continua gravando só `wahaSession`. Linha antiga permanece com NULL e é
 *  lida pelo fallback legado. */
export type StoredWebhook = { workspaceId: string; wahaSession: string; providerSessionId?: string | null; externalEventId: string; eventType: WahaWebhookEvent['event']; occurredAt: string; payload: Record<string, unknown>; receivedAt: string };
...
export class SupabaseWahaWebhookStore implements WahaWebhookStore, ConversationStore, ReactionStore {
  ...
  private async persistEvent(event: StoredWebhook, options: PersistOptions): Promise<IngestResult> {
    if (options.storeEvent) {
      const { error } = await this.client.from('waha_webhook_events').insert({ workspace_id: event.workspaceId, waha_session: event.wahaSession, ...(event.providerSessionId ? { provider_session_id: event.providerSessionId } : {}), external_event_id: event.externalEventId, event_type: event.eventType, occurred_at: event.occurredAt, payload_json: sanitize(event.payload), received_at: event.receivedAt });
