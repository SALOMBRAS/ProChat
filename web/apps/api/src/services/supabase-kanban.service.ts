...
export class SupabaseKanbanService {
  ...
  async conversations(workspaceId: string, boardId: string, stageId: string, page: number, pageSize: number, filters: KanbanFilters) {
    ...
    const config = rows.some((row: any) => row.conversation_sla_metrics) ? await this.sla.config(workspaceId) : undefined;
    return { items: rows.map((row: any) => { const metric = Array.isArray(row.conversation_sla_metrics) ? row.conversation_sla_metrics[0] : row.conversation_sla_metrics; return { conversationId: row.conversation_id, stageId: row.stage_id, position: Number(row.position), updatedAt: row.updated_at, maskedId: row.conversations.chat_id.replace(/\d(?=\d{4})/g, '•'), lastMessage: (row.conversations.last_message ?? '').slice(0, 180), lastMessageAt: row.conversations.last_message_at, unreadCount: row.conversations.unread_count, conversationType: row.conversations.conversation_type, assignedUserId: row.conversations.assigned_user_id, assignedTeamId: row.conversations.assigned_team_id, routingQueueId: row.conversations.routing_queue_id, priority: row.conversations.priority, tags: row.conversation_metadata?.tags ?? [], slaStatus: metric?.sla_status ?? null, sla: metric && config ? projectSlaCard({ slaStatus: metric.sla_status, waitingSinceAt: metric.waiting_since_at, firstResponseAt: metric.first_response_at, frozenAt: metric.frozen_at }, config) : null }; }), page, pageSize, total: count ?? 0 };
  }
