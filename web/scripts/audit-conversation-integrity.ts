...
if (apply) for (const row of reviewed.filter(row => row.classification === 'technical' || row.classification === 'probable_false_direct' || row.classification === 'valid_direct')) { const visibility = row.classification === 'technical' ? 'technical' : row.classification === 'probable_false_direct' ? 'quarantined' : 'visible'; const { error } = await db.from('conversations').update({ visibility_state: visibility, integrity_classification: row.classification, integrity_reason_safe: row.reason, integrity_reviewed_at: new Date().toISOString() }).eq('workspace_id', row.workspace_id).eq('id', row.conversation_id); if (error) throw error; }
