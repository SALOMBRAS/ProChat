...
// Reactions follow WhatsApp Web parity: one reaction per author per message,
// toggled off by repeating the same emoji. reactorWhatsappId stays internal to
// the API contract and is never rendered as a raw identifier by the dashboard.
...
export const inboxMessageSchema = z.object({ id: z.string().min(1), direction: z.enum(['inbound', 'outbound']), content: z.string().nullable(), timestamp: z.string().datetime(), status: z.enum(['sending', 'received', 'sent', 'delivered', 'read', 'failed']), messageType: z.string().min(1), chatId: z.string().min(1), senderWhatsappId: z.string().min(1).nullable().optional(), mediaUrl: z.string().url().nullable().optional(), mediaMimeType: z.string().nullable().optional(), mediaFilename: z.string().nullable().optional(), mediaSize: z.number().int().nonnegative().nullable().optional(), thumbnailUrl: z.string().url().nullable().optional(), duration: z.number().int().nonnegative().nullable().optional(), quotedMessageId: z.string().nullable().optional(), metadata: z.record(z.unknown()), reactions: z.array(messageReactionSchema).default([]) });
...
export const whatsappMentionJidSchema = z.string().regex(/^\d{6,20}@(c\.us|lid)$/);
export const internalSendMessageCommandSchema = z.object({ type: z.literal('message.send'), payload: z.object({ wahaSession: z.string().trim().min(1).max(200), chatId: z.string().trim().min(1).max(200), text: z.string().trim().min(1).max(4_096), mentions: z.array(whatsappMentionJidSchema).max(50).optional() }) });
...
// An empty reaction string removes the reaction, matching the WAHA protocol.
export const internalSendReactionCommandSchema = z.object({ type: z.literal('message.sendReaction'), payload: z.object({ wahaSession: z.string().trim().min(1).max(200), chatId: z.string().trim().min(1).max(200), messageId: z.string().trim().min(1).max(200), reaction: z.string().max(32) }) });
...
export const internalTransportCommandSchema = z.discriminatedUnion('type', [internalTransportPingCommandSchema, internalListSessionsCommandSchema, internalCreateSessionCommandSchema, internalSessionCommandSchema, internalSendMessageCommandSchema, internalSendAttachmentCommandSchema, internalSendReactionCommandSchema, internalIdentitySyncCommandSchema, internalHistoryPageCommandSchema]);
